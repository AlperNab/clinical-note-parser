...
**Domain:** `Medical / Healthcare Data` · **Suite:** `Medical & Research Suite` · **Accent:** `#5eead4`
...
**Clinical Note Parser** is a standalone, browser-based AI workflow app for **Medical / Healthcare Data**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `clinical_notes`<br>Clinical notes | textarea | Yes | — | Affects input: Clinical notes. |
...
| `specialty`<br>specialty | select | No | general | Affects customization: specialty. |
| `coding_system`<br>coding system | text | No | — | Affects customization: coding system. |
| `phi_mode`<br>PHI mode | select | No | cloud allowed | Affects customization: PHI mode. |
| `local_only_mode`<br>local-only mode | select | No | — | Affects customization: local-only mode. |
| `evidence_threshold`<br>evidence threshold | slider | No | 50 | Affects customization: evidence threshold. |
| `patient_age_group`<br>patient age group | text | No | — | Affects customization: patient age group. |
...
- **specialty**: general, ENT, cardiology, internal medicine, pediatrics, psychiatry, custom
- **PHI mode**: cloud allowed, redact sensitive fields, local only
...
uvicorn app.main:app --host 127.0.0.1 --port 9107
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9107`. |
...
```bash
curl http://127.0.0.1:9107/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9107/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9107 uvicorn app.main:app --host 0.0.0.0 --port 9107
```
...
- `plugins/clinical-note-parser.json` for plugin metadata.
